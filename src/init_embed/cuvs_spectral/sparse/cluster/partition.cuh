 ...
 *    Cost = \f$sum_i\f$ (Edges cut by ith partition)/(Vertices in ith
 * partition)
 *
...
std::tuple<vertex_t, weight_t, vertex_t> partition(
  ...
  return cuvs::spectral::detail::partition<vertex_t, weight_t, EigenSolver,
                                           ClusterSolver>(
 ...
 *  function:
 *    Cost = \f$sum_i\f$ (Edges cut by ith partition)/(Vertices in ith
 * partition) Graph is assumed to be weighted and undirected.
 *
 *  @param handle raft handle for managing expensive resources
 *  @param csr_m Weighted graph in CSR format
