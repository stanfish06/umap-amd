...
struct eigen_solver_config_t {
  ...
  unsigned long long seed{
      1234567}; // CAVEAT: this default value is now common to all instances of
                // using seed in Lanczos; was not the case before: there were
                // places where a default seed = 123456 was used; this may
                // trigger slightly different # solver iterations
};
...
struct lanczos_solver_t {
  ...
  index_type_t solve_smallest_eigenvectors(
      ...
      raft::spectral::matrix::sparse_matrix_t<index_type_t, value_type_t> const
          &A,
    ...
    index_type_t iters{};
    raft::sparse::solver::computeSmallestEigenvectors(
        handle, A, config_.n_eigVecs, config_.maxIter, config_.restartIter,
        config_.tol, config_.reorthogonalize, iters, eigVals, eigVecs,
        config_.seed);
    return iters;
  }
  ...
  index_type_t solve_largest_eigenvectors(
    ...
    RAFT_EXPECTS(eigVecs != nullptr, "Null eigVecs buffer.");
    index_type_t iters{};
    raft::sparse::solver::computeLargestEigenvectors(
        handle, A, config_.n_eigVecs, config_.maxIter, config_.restartIter,
        config_.tol, config_.reorthogonalize, iters, eigVals, eigVecs,
        config_.seed);
    return iters;
