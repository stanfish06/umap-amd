...
template <typename T>
void fit_embedding(raft::resources const &handle, int *rows, int *cols, T *vals,
                   int nnz, int n, int n_components, T *out,
                   unsigned long long seed = 1234567, T tol = 0.01) {
  ...
  raft::sparse::convert::coo_to_csr(handle, rows, cols, vals, nnz, n,
                                    src_offsets.data(), dst_cols.data(),
                                    dst_vals.data());
  ...
  struct no_op_cluster_solver_t {
    ...
    std::pair<value_type_t, index_type_t>
    solve(raft::resources const &handle, size_type_t n_obs_vecs,
  ...
  cuvs::spectral::partition(handle, r_csr_m, eig_solver,
