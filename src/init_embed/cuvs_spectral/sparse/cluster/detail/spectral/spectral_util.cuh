...
// TODO: Expose needed wrappers in RAFT's public API so we don't need to call
// detail APIs in cuVS
...
void transform_eigen_matrix(raft::resources const &handle, edge_t n,
  ...
  for (auto i = 0; i < nEigVecs; ++i) {
    ...
    RAFT_CUBLAS_TRY(raft::linalg::detail::cublasnrm2(
        cublas_h, n, eigVecs + IDX(0, i, n), 1, &std, stream));
  ...
  //   TODO: in-place transpose
  {
    ...
    // TODO: Call from public API when ready
    RAFT_CUBLAS_TRY(raft::linalg::detail::cublasgeam(
        cublas_h, CUBLAS_OP_T, CUBLAS_OP_N, nEigVecs, n, &one, eigVecs, n,
        &zero, (weight_t *)NULL, nEigVecs, work.raw(), nEigVecs, stream));

    RAFT_CUDA_TRY(cudaMemcpyAsync(eigVecs, work.raw(),
 ...
 */
template <typename index_type_t, typename value_type_t> struct equal_to_i_op {
  const index_type_t i;

public:
  equal_to_i_op(index_type_t _i) : i(_i) {}
  template <typename Tuple_> __host__ __device__ void operator()(Tuple_ t) {
    thrust::get<1>(t) =
...
bool construct_indicator(
    raft::resources const &handle, edge_t index, edge_t n,
  ...
  // TODO: Call from public API when ready
  RAFT_CUBLAS_TRY(raft::linalg::detail::cublasdot(
      cublas_h, n, Bx.raw(), 1, part_i.raw(), 1, &partStats, stream));
